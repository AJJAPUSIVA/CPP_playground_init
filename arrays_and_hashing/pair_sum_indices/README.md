...
Return indices of two distinct elements whose values add to the target. Return {-1,-1} if no pair exists.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(n^2) | O(n) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n) expected | O(n) |

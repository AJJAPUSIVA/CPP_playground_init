...
Return an array where output[i] is the product of every input element except input[i], without division in the optimal solution.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(n^2) | O(1) excluding output |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n) | O(1) excluding output |

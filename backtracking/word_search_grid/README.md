...
Return whether a word can be formed by adjacent horizontal/vertical cells without reusing a cell.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(m*n*4^L) | O(L) recursion |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(m*n*4^L) with frequency pruning | O(L) recursion |

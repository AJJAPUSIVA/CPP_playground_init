...
Return floor(sqrt(x)) for a non-negative integer x without using floating-point arithmetic in the optimal solution.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(sqrt x) | O(1) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(log x) | O(1) |

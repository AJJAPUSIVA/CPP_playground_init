...
Given pile sizes and h hours, return the minimum positive integer speed that finishes all piles within h hours.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(max(piles)*n) | O(1) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n log max(piles)) | O(1) |

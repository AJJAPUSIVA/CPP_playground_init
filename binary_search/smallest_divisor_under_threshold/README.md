...
Find the smallest positive divisor such that the sum of ceil(nums[i]/divisor) is at most threshold.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(max(nums)*n) | O(1) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n log max(nums)) | O(1) |

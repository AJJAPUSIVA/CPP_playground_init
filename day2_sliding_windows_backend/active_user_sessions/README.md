...
Given session intervals and query timestamps, return the number of active sessions at each query time using [start,end) semantics.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(sessions*queries) | O(s) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O((s+q) log s) | O(s) |

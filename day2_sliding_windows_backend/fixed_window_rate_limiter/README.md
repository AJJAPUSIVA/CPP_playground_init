...
Process (key,timestamp) requests and allow at most max_requests per fixed window of window_size seconds.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(log keys) per request | O(keys) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(1) expected per request | O(keys) |

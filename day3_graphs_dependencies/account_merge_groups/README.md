...
Merge account records that share an email; each record starts with a name followed by emails.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(n^3 * emails) | O(total emails) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(total emails alpha(n)) | O(total emails) |

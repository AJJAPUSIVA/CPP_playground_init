...
Count assignments of plus/minus signs to values so the resulting expression equals target.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(2^n) | O(reachable_sums) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n * reachable_sums) | O(reachable_sums) |

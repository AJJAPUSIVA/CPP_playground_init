...
Given edges of a tree plus one extra edge, return the edge whose removal restores a tree.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(E*(V+E)) | O(V+E) / O(V) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(E alpha(V)) | O(V+E) / O(V) |

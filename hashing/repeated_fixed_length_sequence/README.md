# Repeated Fixed Length Sequence

## Problem Description

Return all repeated substrings of fixed length k, each reported once.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(n^2*k) | O(n*k) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n*k) | O(n*k) |

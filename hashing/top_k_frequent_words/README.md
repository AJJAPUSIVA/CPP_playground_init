...
Return the k most frequent words ordered by decreasing frequency and lexicographically for ties.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(n + u log u) | O(u+k) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n + u log k) | O(u+k) |

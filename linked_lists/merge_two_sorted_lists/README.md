...
Merge two sorted singly linked lists and return a sorted list.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O((n+m) log(n+m)) | O(n+m) output nodes |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n+m) | O(n+m) output nodes |

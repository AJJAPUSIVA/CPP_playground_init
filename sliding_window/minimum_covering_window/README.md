...
Return the minimum substring of s containing every character of t with multiplicity, or an empty string if none exists.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(n^3 + alphabet) | O(alphabet) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(n + alphabet) | O(alphabet) |

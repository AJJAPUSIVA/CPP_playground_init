...
Return the inorder successor value of a target value in a binary search tree, or an empty optional if none.

## Approaches

| File | Approach | Time Complexity | Extra Space |
|---|---|---:|---:|
| `solution_brute_force.cpp` | Straightforward / baseline solution | O(n) | O(n) / O(1) |
| `solution_optimal.cpp` | Optimized interview-oriented solution | O(height) | O(n) / O(1) |
